...
object ErrorLintHandler : ProblemHandler() {
    private fun fixErrorfVerb(file: PsiFile, document: Document, issue: LintIssue, overrideLine: Int): Pair<Array<IntentionAction>, TextRange?> {
            ...
            when (element) {
                is GoCallExpr ->
                    if (element.expression.text.startsWith("fmt.Errorf"))
                        return arrayOf<IntentionAction>(GoWrapErrorFormatFix(element, pos)) to element.argumentList.expressionList.first().textRange
            ...
            when {
                ...
                issue.Text == "type assertion on error will fail on wrapped errors. Use errors.As to check for specific errors" ->
                    chainFindAndHandle(file, document, issue, overrideLine) { element: GoTypeAssertionExpr ->
                        ...
                        if (parent is GoShortVarDeclaration && parent.varDefinitionList.size == 2)
                            arrayOf<IntentionAction>(GoErrorTypeAssertionFix(element, document.getLineStartOffset(overrideLine))) to element.textRange
                ...
                issue.Text == "switch on an error will fail on wrapped errors. Use errors.Is to check for specific errors" ->
                    chainFindAndHandle(file, document, issue, overrideLine) { element: GoSwitchStatement ->
                        arrayOf<IntentionAction>(GoBringToExplanationFix("https://golang.org/pkg/errors/#Is", "Example & Best practice")) to element.switchStart?.textRange
                    }
                issue.Text == "type switch on error will fail on wrapped errors. Use errors.As to check for specific errors" ->
                    chainFindAndHandle(file, document, issue, overrideLine) { element: GoSwitchStatement ->
                        arrayOf<IntentionAction>(GoBringToExplanationFix("https://golang.org/pkg/errors/#As", "Example & Best practice")) to element.switchStart?.textRange
                    }
                issue.Text.startsWith("comparing with") ->
                    chainFindAndHandle(file, document, issue, overrideLine) { element: GoConditionalExpr ->
                        ...
                        arrayOf<IntentionAction>(GoReplaceWithErrorsIsFix(element)) to element.textRange
                    }
                else ->
                    NonAvailableFix
            }
